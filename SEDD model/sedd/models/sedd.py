...
# Transformer model with sin, cos embedding
...
class SEDD(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config, vocab_size):
        ...
        self.style_dim = config['model'].get('style_dim', 2048)
        
        # self.fusion = PerFuMe(768)
        # Add vector quantizer for style codebook
        if self.use_style_embedding:
            self.style_quantizer = VectorQuantizer(num_embeddings=2, embedding_dim=1024)
        else:
    ...
    def forward(self, indices, sigma, use= None, style=None, target = None):
        # Compute embeddings

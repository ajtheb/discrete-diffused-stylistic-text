...
def query_model_batch(pipeline, system_message, user_messages, temperature=0.3, max_length=256, batch_size=16):
    start_time = time()
    
    prompts = [
        pipeline.tokenizer.apply_chat_template(
            [{"role": "system", "content": system_message}, {"role": "user", "content": msg}],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inference llama 3.1 for counterspeech generation")
    parser.add_argument("--input_file", type=str, required=True, help="Path to input CSV file")
    parser.add_argument("--model_id", type=str, default = "/home/models/Meta-Llama-3-8B-Instruct", help="Path to input CSV file")
    ...
    # Choose prompt type: "zero-shot","few-shot"
    prompt_type = args.prompt_type
    num_examples = 2  # Number of examples for few-shot learning
    ...
    # Run model
    # system_message = "You are an AI assistant designed to generate counterspeech for the given hatespeech."
    system_message = ""
    ...
    res.to_csv(f"results/{prompt_type}/llama.csv", index=False)

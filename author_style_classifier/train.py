...
df=pd.read_csv("input/Person_specific_counterspeech.csv")

# Text
sentences = df['Counterspeech']
# Labels
labels = df['Style'].map({"Gandhi":0, "Mandela":1})
...
for fold,(idxT,idxV) in enumerate(skf.split(roberta_input_ids.numpy(), labels.numpy())):
    ...
    roberta_model = RobertaForSequenceClassification.from_pretrained("/home/models/roberta-large", # 12-layer, 768-hidden, 12-heads, 125M parameters RoBERTa using the BERT-base architecture
                                                                    ...
                                                                    hidden_dropout_prob=0.3,  # Default is 0.1, increase for stronger regularization
    ...
    #                                    token_type_ids=None, 
        ...
        if(fold==9 and avg_val_loss<min_prev_loss):
            min_prev_loss = avg_val_loss
            #model save
            torch.save(roberta_model.state_dict(), f'saved_models/model_gandhi_style_epoch_{epoch_i + 1}_fold{fold}.pth')
    print("")
...
print(f"Average F1 score across all folds: {avg_f1_score_all_epochs:.4f}")